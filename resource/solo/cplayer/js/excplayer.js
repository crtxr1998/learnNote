...
function musicInfo(arg) {
	...
	if (sessionStorage.getItem("musics")) {
		...
		Ajax({
			...
			success: (res) => {
				...
				item = {
					...
					"lyric": lyrics(musics.data.url.substring(musics.data.url.indexOf("=") + 1, musics.data.url.lastIndexOf(
						".")))
				}
				list.push(item);
